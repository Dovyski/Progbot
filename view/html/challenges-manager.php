	...
	echo '<form action="challenges-manager.php" method="post" name="formChallenges" id="formChallenges">';
		echo '<div class="row">';
			...
			echo '<div class="col-md-3">';
				...
				echo ' <a href="categories-manager.php"><span class="fa fa-edit"></span></a>';

				echo '<select name="fk_category" class="col-lg-4 form-control">';
					foreach($aCategories as $aCategoryId => $aCategoryInfo) {
						echo '<option value="'. $aCategoryId.'" '.($aChallengeInfo['fk_category'] == $aCategoryId ? 'selected="selected"' : '').'>'.$aCategoryInfo['name'].'</option>';
					}
				echo '</select>';
